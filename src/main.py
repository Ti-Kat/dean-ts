...
from data_persistence import load_dataset, create_result_dir, store_results, store_general_information
...
dataset = np.delete(complete_dataset, obj=[0, 2], axis=1)
...
for i in range(0, config['ensemble_size']):
    ...
    # Randomly select previous time steps
    lag_indices = np.random.choice(range(1, config['look_back']),
                                   size=config['bag'] - 1,
                                   replace=False)

    # Add values from chosen time steps as features
    transformed_set = add_lag_features(dataset, lag_indices)
    ...
    submodel = build_submodel([config['bag'] for _ in range(config['depth'])],
                              reg=None,
                              act='relu',
                              mean=1.0)
    ...
    # Save results
    store_results(result_sub_dir, y_score, auc_score, lag_indices)
